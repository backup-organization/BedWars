...
class UpgradeShop {
    ...
    public const UPGRADE_DESCRIPTION = [
     ...
     */
    public static function handleTransaction(int $formId, Player $player, BedWars $plugin) : void {
        ...
        foreach ($playerTeam->getPlayers() as $player) {
            ...
            foreach([$helmet, $chestplate, $leggings, $boots] as $armor){
                $armor->addEnchantment(new EnchantmentInstance(Enchantment::getEnchantmentByName(Enchantment::PROTECTION)), $playerTeam->getUpgrade('armorProtection'));
            }
    ...
    /**
     * @param int $selectedUpgrade
     * @param Player $player
     * @param BedWars $plugin
     */
    public static function sendBuyPage(int $selectedUpgrade, Player $player, BedWars $plugin) : void {
        $playerTeam = $plugin->getPlayerTeam($player);
        ...
        } else if($playerTeam->getUpgrade($upgradeData['identifier']) < self::MAX_LEVELS[$upgradeData['identifier']] && $player->getInventory()->contains(Item::get(Item::DIAMOND, 0, $cost))){
        ...
        } else if($playerTeam->getUpgrade($upgradeData['identifier']) < self::MAX_LEVELS[$upgradeData['identifier']]){
